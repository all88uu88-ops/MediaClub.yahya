...
import { AppState, User, Post, ClubLocation, Comment, Theme, Idea, IdeaType, Specialization, UserStatus, Notification, Mood, BadgeDefinition, Story } from './types';
import { INITIAL_USERS, INITIAL_LOCATIONS, INITIAL_POSTS, INITIAL_IDEAS, BADGE_DEFINITIONS, SOUNDS, INITIAL_STORIES } from './constants';
...
const App: React.FC = () => {
  const [state, setState] = useState<AppState>(() => {
    ...
    return {
      ...
      notifications: [],
      currentUser: null,
      theme: 'dark' // Force dark for cinematic vibe
    };
  });

  const [view, setView] = useState<'FEED' | 'MEMBERS' | 'LOCATIONS' | 'PROFILE' | 'ADMIN' | 'IDEAS' | 'CANVAS' | 'TOUR'>('FEED');
  ...
  // ... (User Updates, Post Creation, etc. - same logic as before)
  ...
  const handleCreatePost = async () => {
    ...
    // Check if user is previewing before actual creation, usually handled by separate button, but here we just handle the actual creation
    setIsUploading(true);
    try {
        ...
        <div className={`transition-all duration-1000 ${loginPhase === 'REVEAL' || showPreview ? 'blur-sm scale-[0.98] opacity-50 pointer-events-none grayscale-[50%]' : ''}`}>
          ...
          <div className="flex flex-col lg:flex-row min-h-screen">
            ...
            <aside className="hidden lg:flex w-24 flex-col items-center fixed inset-y-0 right-0 z-50 py-8">
              <div className="glass-panel w-16 flex-1 rounded-[2rem] flex flex-col items-center py-6 gap-6 shadow-2xl backdrop-blur-2xl bg-black/40 border border-white/10">
                ...
                <nav className="flex-1 flex flex-col gap-4 w-full px-2">
                  {[ { id: 'FEED', icon: LayoutGrid, label: 'Feed' }, { id: 'IDEAS', icon: Lightbulb, label: 'Ideas' }, { id: 'MEMBERS', icon: Users, label: 'Crew' }, { id: 'LOCATIONS', icon: MapPin, label: 'Sets' }, { id: 'CANVAS', icon: Palette, label: 'Art' }, { id: 'TOUR', icon: Compass, label: 'Tour' }, { id: 'ADMIN', icon: ShieldCheck, label: 'Admin', adminOnly: true }, { id: 'PROFILE', icon: UserCircle, label: 'Me' } ].map(item => {
            ...
            <div className="lg:hidden fixed top-0 left-0 right-0 z-50 p-4">
              ...
              {mobileMenuOpen && (
                  <div className="glass-panel mt-2 rounded-2xl p-2 absolute left-4 right-4 flex flex-col gap-1 animate-in slide-in-from-top-2 bg-black/90 backdrop-blur-xl border border-white/10">
                    {[ { id: 'FEED', icon: LayoutGrid, label: 'Feed' }, { id: 'IDEAS', icon: Lightbulb, label: 'Ideas' }, { id: 'MEMBERS', icon: Users, label: 'Crew' }, { id: 'LOCATIONS', icon: MapPin, label: 'Sets' }, { id: 'TOUR', icon: Compass, label: 'Studio Tour' }, { id: 'PROFILE', icon: UserCircle, label: 'Profile' } ].map(item => (
            ...
            <main className="flex-1 lg:mr-24 p-6 lg:p-10 pt-24 lg:pt-10 overflow-x-hidden relative z-10">
                    ...
                    <div className="relative">
                      ...
                      {showNotifications && (
                        <div className="absolute top-full right-0 mt-2 z-[70]">
                           {/* Content handled by NotificationCenter component via absolute positioning in parent, or we can portal it. 
                               For this structure, passing props to the layout component is easier. 
                               We rendered NotificationCenter outside of this header div to avoid z-index clipping if any parents have overflow hidden.
                           */}
                        </div>
                      )}
                    </div>
              ...
              <div className="max-w-6xl mx-auto reveal-cinematic">
                {view === 'FEED' && (
                  <div className="space-y-12">
                    ...
                    <div className="relative">
                      ...
                      <div className="flex items-center justify-between mb-6">
                          ...
                          <button className="text-[10px] font-bold text-indigo-400 hover:text-white transition uppercase tracking-widest">View All Archive</button>
                      </div>
